...
export default function FormDialog() {
  ...
  const createCard = async () => {
    await axios.post(`http://localhost:3333/api/${this.props.idDashboard}`,{text: "text"}, {headers: {'Authorization' : `Bearer ${bearerToken}`}})
  };
    ...
    <div>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogActions>
          ...
          <Button onClick={createCard}>Subscribe</Button>
        </DialogActions>
      </Dialog>
    </div>
  );
